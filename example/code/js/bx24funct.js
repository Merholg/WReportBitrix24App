...
//	second: 'numeric'
	...
	<tr>
	    <td bgcolor=\"#666666\" onclick=\'MakeReport(0);\'>Электроика и Вентиляция</td>
	</tr>
	</table>`,
...
function GetSTasksCRM()
{
	...
	{
	    var element = document.body.arrDATA.TasksID[document.body.timerNumTick].ID;
	    BX24.callMethod('tasks.task.get', 
	    {taskId: element, select: 'UF_CRM_TASK'}, 
	    function(res)
	    {
		if (typeof(res.answer.result.task.id) != "undefined" && typeof(res.answer.result.task.ufCrmTask) == "object")
		{
		    var taskid = Number(res.answer.result.task.id);
		    var taskidx = document.body.arrDATA.TasksID.findIndex(item => item.ID == taskid);
		    if(taskidx >= 0)
		    {
			res.answer.result.task.ufCrmTask.forEach(element =>
			    document.body.arrDATA.TasksID[taskidx].UF_CRM_TASK.push(Number(element.substring(2))));
		    }
		}
	...
	{
//console.log(document.body.arrDATA.TasksID, document.body.TasksIDsemafor, document.body.arrDATA.Num);
